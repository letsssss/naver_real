...
export async function GET(req: NextRequest, { params }: { params: { seller_id: string } }) {
  ...
  console.log("[✅ 대리티켓팅 통계 API 진입] seller_id:", seller_id);
  ...
  try {
    ...
    if (error) {
      console.error("❌ 대리티켓팅 통계 조회 오류:", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    console.log("✅ 대리티켓팅 통계 조회 성공:", data);
    ...
    if (!data) {
      return NextResponse.json({
        confirmed_count: 0,
        cancelled_count: 0,
        proxy_ticketing_rate: 0
      });
    }

    return NextResponse.json(data);
  } catch (err) {
    console.error("❌ 대리티켓팅 통계 API 오류:", err);
