  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // 임시 메시지 데이터 (추후 실제 메시지 API 연동 필요)
        const dummyMessages = [
          {
            id: 1,
            senderId: formattedData.seller.id,
            text: `안녕하세요! ${formattedData.ticket.title} 티켓 구매해주셔서 감사합니다. 취켓팅 진행 중이니 조금만 기다려주세요.`,
          ...
          },
          {
            ...
            timestamp: new Date(new Date(purchaseData.created_at).getTime() + 5 * 60000).toISOString(),
          },
          {
            id: 3,
            senderId: formattedData.seller.id,
            text: "보통 공연 1-2일 전에 완료되지만, 취소표가 빨리 나오면 더 일찍 완료될 수도 있어요. 상황에 따라 다를 수 있으니 메시지로 안내해 드릴게요.",
            timestamp: new Date(new Date(purchaseData.created_at).getTime() + 10 * 60000).toISOString(),
          },
        ]
        
        // 취켓팅 완료 상태면 추가 메시지 표시
        if (formattedData.currentStep === "ticketing_completed" || formattedData.currentStep === "confirmed") {
          dummyMessages.push({
            id: 4,
            senderId: formattedData.seller.id,
            text: "안녕하세요! 취켓팅이 완료되었습니다. 티켓 정보를 확인하시고 구매확정 버튼을 눌러주세요.",
            timestamp: formattedData.stepDates.ticketing_completed || new Date().toISOString(),
          })
          
          if (formattedData.currentStep === "confirmed") {
            dummyMessages.push({
              id: 5,
              senderId: "user",
              text: "네, 확인했습니다. 감사합니다!",
              timestamp: formattedData.stepDates.confirmed || new Date().toISOString(),
            })
          }
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md">
          ...
          <Link href="/mypage">
            <Button className="w-full">마이페이지로 돌아가기</Button>
          </Link>
        </div>
      </div>
    )
